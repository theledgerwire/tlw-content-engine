...
# ── Fix 4: ig_posted ──
for old_ig, new_ig, label in [
    ("if not ig_posted:", "if True:  # v18.1b: always single image for IG", "ig_posted ref"),
    ("ig_posted_skip = True  # v18.1: skip PDF carousel for IG", "# v18.1b: IG always single image", "ig_posted_skip"),
    ("ig_posted = False\n", "", "ig_posted init"),
    ("ig_posted = True\n", "", "ig_posted set"),
]:
    if old_ig in code:
        code = code.replace(old_ig, new_ig, 1); patches += 1; print(f"✓ Fixed {label}")
