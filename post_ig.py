...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-url", required=True, help="Public URL of the image")
    parser.add_argument("--caption", required=True, help="Instagram caption text")
    parser.add_argument("--buffer-key", default=BUFFER_API_KEY, help="Buffer API key")
    parser.add_argument("--profile-ig", default=BUFFER_PROFILE_IG, help="Buffer IG profile ID")
    args = parser.parse_args()

    if not args.buffer_key:
        print("ERROR: No BUFFER_API_KEY"); sys.exit(1)
    if not args.profile_ig:
        print("ERROR: No BUFFER_PROFILE_IG"); sys.exit(1)
