...
# ── UNSPLASH SEARCH API ───────────────────────────────────────────
def fetch_photo_search(keyword):
    """
    Use Unsplash SEARCH endpoint — returns contextually
    relevant photos instead of random ones.
    Picks randomly from top 10 results for variety.
    """
    try:
        r = requests.get(
        ...
        print(f"Unsplash search [{keyword}]: {r.status_code}")
        if r.status_code != 200:
            return None
        data    = r.json()
...
def get_unsplash_photo(keyword):
    if not UNSPLASH_KEY:
        return None
    # Try specific keyword first, then fallbacks
    for kw in [keyword] + UNSPLASH_FALLBACKS:
        photo = fetch_photo_search(kw)
        if photo:
            return photo
    print("All Unsplash searches failed — navy fallback")
    return None
...
def post_to_buffer(post_text, image_url, channel_id, api_key,
    ...
    # Use JSON variables instead of string interpolation
    # This completely avoids all escaping issues with newlines/quotes
    variables = {
        "text":      post_text,
    ...
    if first_comment:
        query = """
mutation CreatePost($text: String!, $channelId: String!, $imageUrl: String!, $comment: String!) {
  createPost(input: {
    text: $text,
    channelId: $channelId,
    ...
    ... on MutationError { message }
        ...
        variables["comment"] = first_comment
    else:
        query = """
mutation CreatePost($text: String!, $channelId: String!, $imageUrl: String!) {
  createPost(input: {
    text: $text,
    channelId: $channelId,
    schedulingType: automatic,
